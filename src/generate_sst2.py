"""
Synthetic Data Generator for SST-2 (Optimised for Speed)
--------------------------------------------------------------
Features:
- Batch Generation (requests X reviews per LLM call)
...
Task: Classify movie reviews into 2 sentiments: Negative (0) or Positive (1)

Instructions:
Generate exactly {BATCH_SIZE} realistic, diverse, and completely unique short movie reviews for the sentiment: {CATEGORY}

Requirements:
- Write a realistic 1-2 sentence movie review.
- If the category is Negative, make it extremely critical. If Positive, make it glowing.
- Do NOT include explanations, markdown wrappers, or intro text.
- Output your response STRICTLY as a valid JSON array of objects.
- Each object must have "text" (the review) and "label" ({LABEL_ID}).

Example format:
[
  {{"text": "An absolute masterpiece that redefines modern cinema.", "label": 1}},
  {{"text": "A total waste of time with terrible acting.", "label": 0}}
]
"""

# ── Argument parsing ──────────────────────────────────────────────────────────
def parse_args():
    parser = argparse.ArgumentParser(description="Generate synthetic SST-2 data via Ollama (Fast)")
    parser.add_argument("--total-per-class", type=int, default=5000,
                        help="Total examples to generate per category (default: 5000)")
    parser.add_argument("--batch-size", type=int, default=10,
                        help="How many reviews the LLM should write per prompt (default: 10)")
    parser.add_argument("--workers", type=int, default=4,
                        help="Number of parallel request threads (default: 4)")
    parser.add_argument("--output", type=str, default=r"C:\Users\Lenovo\Downloads\gen ai\data\synthetic\synthetic_sst2_10k.csv",
...
def generate_batch(model: str, category: str, batch_size: int):
    label_id = LABEL_MAP[category]
    prompt = PROMPT_TEMPLATE.format(CATEGORY=category, LABEL_ID=label_id, BATCH_SIZE=batch_size)
    payload = {
        "model": model,
        "prompt": prompt,
        "stream": False,
...
def generate_for_category(model: str, category: str, total_n: int, batch_size: int, workers: int):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(generate_batch, model, category, batch_size) for _ in range(workers)}
        ...
        while futures and len(results) < total_n:
            for future in as_completed(list(futures)):
                ...
                if len(results) < total_n:
                    futures.add(executor.submit(generate_batch, model, category, batch_size))
...
def main():
    ...
    print(f"Dataset     : SST-2 Binary Sentiment")
    print(f"Target      : {args.total_per_class} per class ({args.total_per_class * 2} total)")
