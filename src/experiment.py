"""
Synthetic Data Ratio Experiment + Temperature Scaling (Experiment B)
======================================================================
Runs 5 controlled DistilBERT experiments for each synthetic/real mix ratio:
  0%   synthetic  →  10,000 real
  25%  synthetic  →   7,500 real + 2,500 synthetic
  50%  synthetic  →   5,000 real + 5,000 synthetic
  75%  synthetic  →   2,500 real + 7,500 synthetic
  100% synthetic  →  10,000 synthetic

For each ratio, AFTER training:
  - ECE is computed BEFORE Temperature Scaling
  - Temperature T is optimized using 200 real held-out examples
  - ECE is recomputed AFTER scaling
  - Results saved to experiment_results.csv with all metrics
"""
...
VAL_N           = 200       # real examples used for Temperature Scaling
...
RESULTS_FILE    = r"C:\Users\Lenovo\Downloads\gen ai\results\experiment_results.csv"
DIAGRAMS_DIR    = r"C:\Users\Lenovo\Downloads\gen ai\results\figures"
...
# Carve out 200 real examples strictly for Temperature Scaling validation
# These are NEVER used in any training datasets
...
def find_temperature(val_logits: np.ndarray, val_labels: np.ndarray) -> float:
    """
    Learn a single scalar temperature T by minimising cross-entropy NLL
    on the held-out real validation set (200 examples).
    Does NOT use any test set data.
    """
...
for synth_frac, label in RATIOS:
    ...
    output_dir   = f"./checkpoints/experiments/ratio_{ratio_tag}"
    ...
    # Get real logits from TEST set (for ECE)
    test_output  = trainer.predict(test_ds)
    test_logits  = test_output.predictions
    test_labels  = test_output.label_ids

    # Get real logits from VAL set (for Temperature calibration only)
    ...
    plot_reliability_diagram(scaled_logits, test_labels,
                             title=f"After Scaling — {label} (T={T:.3f})",
                             save_path=diagram_after)

    # ── Save raw logits for SATS analysis ──────────────────────────────────
    np.save(os.path.join(LOGITS_DIR, f"distilbert_{ratio_tag}_test_logits.npy"), test_logits)
    np.save(os.path.join(LOGITS_DIR, f"distilbert_{ratio_tag}_test_labels.npy"), test_labels)
    np.save(os.path.join(LOGITS_DIR, f"distilbert_{ratio_tag}_val_logits.npy"), val_logits)
    np.save(os.path.join(LOGITS_DIR, f"distilbert_{ratio_tag}_val_labels.npy"), val_labels)
    print(f"  Logits saved to {LOGITS_DIR}/distilbert_{ratio_tag}_*.npy")
